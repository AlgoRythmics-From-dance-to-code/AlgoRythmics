...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.trim() || !/\S+@\S+\.\S+/.test(email)) {
      toast.error(t('login.errors.email_invalid'));
      return;
    }
          ...
          <div className="w-full max-w-[500px]">
            {!isSubmitted ? (
                ...
                <form onSubmit={handleSubmit}>
                  <div className="mb-8">
                    <label className="font-montserrat font-bold text-black dark:text-white block mb-2 text-sm sm:text-base select-none">
                      {t('login.forgot_password_email_label')}
                    </label>
                    <input
                      type="email"
                      required
                      ...
                      className="w-full font-montserrat h-12 sm:h-14 border-2 border-[#E0E0E0] dark:border-neutral-700 bg-white dark:bg-[#2a2a2a] dark:text-white rounded-lg px-3 sm:px-4 text-base outline-none focus:border-[#36D6BA] transition-colors"
                  ...
                  <Link
                    href={ROUTES.LOGIN}
                    className="block text-center font-montserrat font-bold text-sm hover:underline"
                    style={{ color: '#269984' }}
                  >
              ...
              <div className="text-center">
                ...
                <p className="font-montserrat text-base sm:text-lg mb-8 text-[#666] dark:text-gray-400">
                  {t('login.forgot_password_success_desc')}
                </p>
                <Link
                  href={ROUTES.LOGIN}
                  className="inline-block font-montserrat font-bold text-[#269984] hover:underline"
                >
                  {t('login.forgot_password_back_to_login')}
                </Link>
              </div>
