...
export default function VideoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // The video IDs are like 'bubble-sort-dance', algorithms are 'bubble-sort'
  const algoBaseId = video.id.replace('-dance', '').replace('-folk', '').replace('-waltz', '').replace('-tango', '').replace('-salsa', '').replace('-ballet', '').replace('-flamenco', '').replace('-hip-hop', '').replace('-swing', '');
  
  // Check if we have a description for the base algorithm, else use bit generic
  const description = t(`algorithms.list.${algoBaseId}.description`) || t('videos.hero_description');
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             {['01', '02', '03', '04'].map((num) => (
               <div key={num} className="flex gap-4">
                 <span className="font-montserrat font-bold text-[#269984] opacity-50 text-2xl">{num}</span>
                 <div>
