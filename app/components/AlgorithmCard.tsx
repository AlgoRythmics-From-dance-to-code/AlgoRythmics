...
import { ArrowRight, Clock, Activity, BarChart3, Check } from 'lucide-react';
...
export default function AlgorithmCard({ algorithm, index }: AlgorithmCardProps) {
  ...
  const difficultyColor = 
    algorithm.difficulty === 'Easy' ? 'text-emerald-500 bg-emerald-500/10' :
    algorithm.difficulty === 'Medium' ? 'text-amber-500 bg-amber-500/10' :
    ...
    >
      ...
      <div className="relative aspect-[16/10] w-full bg-[#f8fdfc] dark:bg-[#0d0d0d] flex items-center justify-center p-8 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
           <span className={`text-[10px] font-montserrat font-bold px-2.5 py-1 rounded-lg uppercase tracking-wider backdrop-blur-md ${difficultyColor}`}>
