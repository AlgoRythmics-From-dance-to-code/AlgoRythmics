...
export default function BubbleSortVisualizer({ id = 'bubble-sort' }: { id?: string }) {
  ...
  const steps = useMemo(() => {
    let arr = initialValues.map((v, i) => ({ val: v, id: i }));
    const result: Step[] = [{ array: [...arr], activeIndices: [], swapping: false, sortedIndices: [] }];
    let sorted: number[] = [];

    for (let i = 0; i < arr.length; i++) {
      for (let j = 0; j < arr.length - i - 1; j++) {
        // Step 1: Compare
        result.push({ array: [...arr], activeIndices: [j, j + 1], swapping: false, sortedIndices: [...sorted] });
        ...
        if (arr[j].val > arr[j + 1].val) {
          ...
          arr[j] = arr[j + 1];
          arr[j + 1] = temp;
          result.push({ array: [...arr], activeIndices: [j, j + 1], swapping: true, sortedIndices: [...sorted] });
        }
      }
      // Step 3: Mark largest as sorted
      sorted.push(arr.length - i - 1);
      result.push({ array: [...arr], activeIndices: [], swapping: false, sortedIndices: [...sorted] });
    }
    return result;
  }, [initialValues]);
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="mt-8 flex items-center justify-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-montserrat font-bold text-gray-500 uppercase tracking-widest">Compare</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-orange-500" />
            <span className="text-xs font-montserrat font-bold text-gray-500 uppercase tracking-widest">Swap</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-green-500" />
            <span className="text-xs font-montserrat font-bold text-gray-500 uppercase tracking-widest">Sorted</span>
          </div>
      </div>
